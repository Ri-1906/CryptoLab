...
/* OFB Encryption */
vector<int> encrypt_OFB(vector<int> pt, vector<int> key, vector<int> iv){
    vector<int> cipher;
    ...
    for(int i = 0; i < pt.size(); i += block_size){
        ...
        feedback = encrypt_block(feedback, key);   // keystream

        vector<int> c(block_size);
        for(int j = 0; j < block_size; j++)
            c[j] = block[j] ^ feedback[j];
...
int main(){
    ...
    vector<int> cipher = encrypt_OFB(plaintext, key, iv);
